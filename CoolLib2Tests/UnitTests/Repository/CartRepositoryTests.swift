...
final class CartRepositoryTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            ...
            repository = CartRepositoryImpl(modelContext: context)
        } catch {
    ...
    func test_toggleCart_shouldSwitchStates() async throws {
        ...
        let statusAfterAdd = try await repository.isBookInCart(bookId: testBook.id)
        XCTAssertTrue(statusAfterAdd, "Status should be TRUE after first toggle")
        
        // Act 2: Toggle OFF (Remove)
        try await repository.toggleCart(book: testBook)
        let statusAfterRemove = try await repository.isBookInCart(bookId: testBook.id)
        XCTAssertFalse(statusAfterRemove, "Status should be FALSE after second toggle")
    }
    ...
    func test_allCartItems_shouldBeSortedByCreatedAtDescending() async throws {
        ...
        if items.count >= 2 {
            XCTAssertEqual(items[0].id, book2.id, "Most recently added item (ID: \(book2.id)) should be first")
            XCTAssertEqual(items[1].id, book1.id, "Older item (ID: \(book1.id)) should be second")
        }
    }
    ...
    func test_addToCart_duplicatePrevention() async throws {
        ...
        // Assert
        let items = try await repository.allCartItems()
        XCTAssertEqual(items.count, 1, "Duplicate prevention should ensure only one entry exists")
    }
}
